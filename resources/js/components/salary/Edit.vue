...
<!-- start base div -->
    <div>
        <div class="row">
            <router-link :to="{ name: 'salary'}" class="btn btn-primary">Go back</router-link>
        </div>
...
export default{
    ...
    created(){
        ...
        axios.get(globalURL+'/api/edit/salary/'+id)
    ...
    methods: {
        salaryUpdate(){
            let id = this.$route.params.id;
             axios.post(globalURL+'/api/salary/update/'+id, this.form)
            .then(() => {
